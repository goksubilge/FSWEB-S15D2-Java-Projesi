...
public class StringSet {
    public static void main(String[] args) {
        String prg = "Carroll began writing the manuscript of the story the next day, although that earliest version is lost. "
        ...
        String[] word = prg.split(" ");
        System.out.println("Words counts without unique: " + word.length);
        // Set<String> Uniques = new TreeSet<>(Arrays.asList(word));

        Set<Integer> numbers = new TreeSet<>();
        Set<String> Uniques = new TreeSet<>();
        for (String wordd: word) {
            wordd = wordd.replaceAll("[.,!?\"]","");  // wordd =  demelisin, demezsen saçmalıyor.
                    // wordd.replaceAll(".","");
                    // wordd.replaceAll(",","");
                    // wordd.replaceAll("!","");
                    // wordd.replaceAll("?","");  // bunları regex ile tek seferde tanımladım sildirdim.
            try {                                 // isteğe bağlı try catch ile numbers ları pars layıp setten ayırdım.
                numbers.add(Integer.parseInt(wordd));
            } catch (NumberFormatException exception) {
                Uniques.add(wordd.toLowerCase().trim());
            }
        }

        System.out.println("Words counts unique: " + Uniques.size());
